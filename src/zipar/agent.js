...
function sendMemory(name, memory, length, start = 0) {
  let offset = 0;
  while (offset < length) {
...
function _parseAndSendModule(module, relativePath) {
  ...
  if (_encryption === null) {
    ...
    sendData(relativePath, handle2, offsetCryptId);
    buffer.writeU64(0);
    sendMemory(relativePath, buffer, 4, offsetCryptId);
    lseek(handle2, offsetCryptId + 4, SEEK_SET);
    const delta1 = offsetCrypt - (offsetCryptId + 4);
    sendData(relativePath, handle2, delta1, offsetCryptId + 4);
    sendMemory(relativePath, module.base.add(offsetCrypt), lengthCrypt, offsetCrypt);
    const delta2 = offsetCrypt + lengthCrypt;
    lseek(handle2, delta2, SEEK_SET);
    sendData(relativePath, handle2, fileSize - delta2, delta2);
  }
  close(handle2);
